...
# Task 1: Import isolated Semantic Encoder core logic and utilities
...
# --- Meaning Validation Logic (Teammate 3) ---
...
def validate_reconstruction(original: str, reconstructed: str, packet: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if orig_negs:
        for neg in orig_negs:
            ...
            if not any(w in recon_lower for w in ["not", "never", "don't", "no", "except", "avoid", "restrictions", "prohibited"]):
...
def encode_message(payload: EncodeRequest):
    ...
    # Delegate to isolated Semantic Encoder logic
    packet = encode_message_logic(payload.message, payload.mode or "normal")
    
    # Calculate real byte sizes
    ...
    # compression_percentage based on transmitted zlib-compressed packet bytes
...
def decode_message(payload: DecodeRequest):
    ...
    reconstructed = decode_packet_logic(payload.packet)
