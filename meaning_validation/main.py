...
# Import the independent validator module
...
class ValidationRequest(BaseModel):
    ...
    semantic_packet_b64: Optional[str] = None
    encode_latency_ms: float = 0.0
    decode_latency_ms: float = 0.0
    mode: str = "low_resource"
...
@app.post("/validate")
def validate_api(req: ValidationRequest):
    packet = {}
    if req.semantic_packet_b64:
        try:
            raw = base64.b64decode(req.semantic_packet_b64).decode("utf-8")
            packet = json.loads(raw)
        except Exception:
            packet = {}

    # Delegate validation directly to validator.py
    verdict = validate(req.original_message, req.reconstructed_message, packet=packet)
    return verdict
